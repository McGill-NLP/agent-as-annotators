...
CONFIG_PATH: Path = importlib.resources.files("llm_annotators.configs").joinpath(MODEL_NAME, CONFIG_FILE_NAME)
...
class EnvArgsParallel(EnvArgs):
    ...
    def make_env(self, action_mapping, exp_dir, exp_task_kwargs: dict = {}, **kwargs):
        ...
        CONFIG_FILE_NAME = os.getenv("EXPLORATION_CONFIG_NAME", "exploration.tasks.json")
        MODEL_NAME = os.getenv("EXPLORATION_MODEL_NAME", "Qwen/Qwen3-32B")
        CONFIG_PATH = importlib.resources.files("llm_annotators.configs").joinpath(MODEL_NAME, CONFIG_FILE_NAME)

        from llm_annotators.exploration import register # noqa: F401
...
def prepare_exploration_backend(backend, reset_instance=True):
    import llm_annotators.exploration.register as register # noqa: F401
