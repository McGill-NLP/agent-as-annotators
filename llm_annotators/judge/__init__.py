...
def get_content_inside_tag(tag: str, response_msg: str, lower=False):
    if lower:
        tag = tag.lower()
...
def create_chat_messages_from_trajectory(
    trajectory, goal=None, use_screenshot=True, use_axtree=True
):
    last_step = trajectory["steps"][-1]

    if goal is None:
        goal = defaults.GOAL_TEMPLATE.format(goal=last_step["goal"])

    if use_screenshot:
        img_msg_content = [{"type": "image_url", "image_url": {"url": last_step["screenshot_base64"]}}]
    else:
        img_msg_content = []

    if use_axtree:
        axtree_msg = defaults.AXTREE_TEMPLATE.format(axtree=last_step["axtree_txt"])
