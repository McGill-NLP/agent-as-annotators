...
from llm_annotators.utils import shorthand_to_model
...
def get_websynth_benchmark(
    reset_instance=True, config_name="websynth.tasks.json", config_anchor="llm_annotators.configs.websynth", llm_judge=None, use_screenshot=True, use_axtree=True
):
    ...
    from llm_annotators.benchmarks.websynth import (
        get_task_ids,
        WebSynthBenchmark,
        get_websynth_config_path,
        make_env_args_list_from_fixed_seeds_parallel,
    )
    config_path = get_websynth_config_path()
    task_ids = get_task_ids(config_path)
    logging.info(f'Using {len(task_ids)} tasks from config file "{config_path}"')

    if not config_name.startswith("websynth."):
        benchmark_name = f"websynth_{config_name.split('.')[0]}"
    else:
...
if __name__ == "__main__":  # necessary for dask backend
    ...
    from llm_annotators.benchmarks.websynth.study import WebSynthStudy
    import llm_annotators.utils as lau
    import llm_annotators.modeling as lam
    ...
    parser.add_argument(
        ...
        help="Select the config name to use for the websynth benchmark",
    )
    parser.add_argument(
        "--disable-reset",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Select the config anchor to use for the websynth benchmark",
    )
    parser.add_argument(
        "--llm-judge-model",
        type=str,
    ...
    if args.benchmark == "websynth":
        ...
        benchmark = get_websynth_benchmark(
