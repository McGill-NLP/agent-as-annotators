...
def get_a3_synth_config_path():
    ...
    CONFIG_ANCHOR = os.getenv("WEBSYNTH_CONFIG_ANCHOR", "agent_as_annotators.configs.a3_synth")
...
class GenericWebSynthTask(AbstractBrowserTask):
    ...
    def setup(self, page: playwright.sync_api.Page) -> tuple[str, dict]:
        ...
        from agent_as_annotators.benchmarks.a3_synth.evaluators import evaluator_router_a3_synth
        ...
        # build the evaluator
        self.evaluator = evaluator_router_a3_synth(self.config_file)
...
class EnvArgsParallel(EnvArgs):
    """
    NOTE: This is needed in order to run the parallel runs with the a3_synth benchmark, i.e. via "ray" with 4 jobs.
    """
    def make_env(self, action_mapping, exp_dir, exp_task_kwargs: dict = {}):
        logger.info("Importing agent_as_annotators.benchmarks.a3_synth.register inside EnvArgsParallel to ensure tasks are registered...")
...
def prepare_a3_synth_backend(backend, reset_instance=True):
    ...
    # as a convention
    if backend != "a3_synth":
        raise ValueError(f"Invalid backend: {backend}. Only 'a3_synth' is supported.")
    ...
    if reset_instance:
        logging.info("Resetting a3_synth instance...")
        default_instance.full_reset()
    else:
        logging.info("Resetting a3_synth instance has been disabled.")
    ...
    task_ids = get_task_ids(get_a3_synth_config_path())
    # N = len(task_ids)
...
class WebSynthBenchmark(Benchmark):
    ...
    def prepare_backends(self):
        ...
        prepare_a3_synth_backend(backend, reset_instance=self.reset_instance)
